...
class StatisticsRepository {
     ...
     */
    async getDailyHistory(userId, metric, days = 30, moduleId = null) {
     ...
     */
    async getPeriodComparison(userId, metric, days = 30, moduleId = null) {
        const safeDays = Math.min(Math.max(days, 1), 365);
        const result = await this.pool.query(`
      SELECT
    ...
    async resetMetric(userId, metric, moduleId) {
