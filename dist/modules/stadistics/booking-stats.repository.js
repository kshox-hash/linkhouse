...
exports.getBusiestSlots = getBusiestSlots;
exports.getClientStats = getClientStats;
exports.getReviewsStats = getReviewsStats;
exports.getQuotesStats = getQuotesStats;
exports.getPortalStats = getPortalStats;
...
async function getRevenueStats(userId) {
    ...
    const [summaryRes, trendRes] = await Promise.all([
       ...
       ORDER BY DATE_TRUNC('month', paid_at)`, [userId]),
    ]);
...
async function getConversionStats(userId) {
    const pool = db_configuration_1.default.getPool();
...
async function getBusiestSlots(userId) {
    const pool = db_configuration_1.default.getPool();
    const result = await pool.query(`SELECT
...
async function getClientStats(userId) {
    const pool = db_configuration_1.default.getPool();
    const result = await pool.query(`WITH client_counts AS (
...
async function getReviewsStats(userId) {
    const pool = db_configuration_1.default.getPool();
    const [summaryRes, recentRes] = await Promise.all([
        pool.query(`SELECT ROUND(AVG(rating)::numeric, 1) AS average, COUNT(*) AS total
       FROM reviews WHERE user_id = $1`, [userId]),
        pool.query(`SELECT rating, comment, client_name, created_at
...
async function getQuotesStats(userId) {
    const pool = db_configuration_1.default.getPool();
    const result = await pool.query(`SELECT
...
async function getPortalStats(userId) {
    const pool = db_configuration_1.default.getPool();
    const result = await pool.query(`SELECT
     ...
     WHERE user_id = $1 AND metric = 'link_opens'`, [userId]);
    const row = result.rows[0];
    return {
