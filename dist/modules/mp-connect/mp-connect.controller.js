...
exports.mpConnectController = void 0;
const mp_connect_service_1 = require("./mp-connect.service");
const mp_connect_repository_1 = require("./mp-connect.repository");
const APP_DEEPLINK = process.env.APP_DEEPLINK_URL ?? "automatiza://auth";
exports.mpConnectController = {
    ...
    async getConnectUrl(req, res) {
        try {
            ...
            const url = (0, mp_connect_service_1.generateMpAuthUrl)(userId);
    ...
    async getStatus(req, res) {
        try {
            ...
            const conn = await (0, mp_connect_repository_1.getMpConnection)(userId);
            if (!conn)
    ...
    async disconnect(req, res) {
        try {
            ...
            if (userId !== authUserId)
                return res.status(403).json({ ok: false, message: "Prohibido" });
            await (0, mp_connect_repository_1.deleteMpConnection)(userId);
    ...
    async callback(req, res) {
        ...
        const userId = (0, mp_connect_service_1.consumeState)(state);
        if (!userId) {
            res.status(400).send(_htmlPage("Error de conexión", "El enlace expiró o no es válido. Vuelve a intentarlo desde la app.", false));
            return;
        }
        try {
            const tokens = await (0, mp_connect_service_1.exchangeCodeForTokens)(code);
            const expiresAt = new Date(Date.now() + tokens.expires_in * 1000);
            await (0, mp_connect_repository_1.saveMpConnection)({
                userId,
            ...
            res.send(_htmlPage("¡Cuenta conectada!", "Tu cuenta de MercadoPago quedó vinculada. Volviendo a la app...", true, `${APP_DEEPLINK}?mp=connected`));
        }
        catch (err) {
...
function _htmlPage(title, message, success, deeplink) {
