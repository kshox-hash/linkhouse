...
exports.GENERATED_PDFS_DIR = void 0;
...
exports.GENERATED_PDFS_DIR = path_1.default.join(__dirname, "..", "generated-pdfs");
if (!fs_1.default.existsSync(exports.GENERATED_PDFS_DIR)) {
    fs_1.default.mkdirSync(exports.GENERATED_PDFS_DIR, { recursive: true });
}
...
function generateQuotePdf(input) {
    return new Promise((resolve, reject) => {
        try {
            const timestamp = Date.now();
            const safeToken = (0, token_1.sanitizeFileName)(input.token);
            const fileName = `cotizacion_${safeToken}_${timestamp}.pdf`;
            const filePath = path_1.default.join(exports.GENERATED_PDFS_DIR, fileName);
            const templateType = input.templateType || "rapida";
            ...
            function drawPartiesBox(startY) {
                const boxH = 90;
            ...
            function ensureSpace(currentY, neededH, drawTableOnNewPage = false) {
                if (currentY + neededH <= pageHeight - 60)
            ...
            // ── Extra fields ─────────────────────────────────────────────────────────
            function drawExtraFields(startY) {
                const ex = input.extraFields || {};
                const rows = [];
                switch (templateType) {
                    case "servicios":
                        if (ex.deliveryDate)
                            rows.push({ label: "Fecha estimada de entrega", value: ex.deliveryDate });
                        if (ex.paymentConditions)
                            rows.push({ label: "Condiciones de pago", value: ex.paymentConditions });
                        if (ex.exclusions)
                            rows.push({ label: "No incluye", value: ex.exclusions });
                        break;
                    case "productos":
                        if (ex.deliveryTime)
                            rows.push({ label: "Tiempo de entrega", value: ex.deliveryTime });
                        if (ex.priceValidity)
                            rows.push({ label: "Validez del precio", value: ex.priceValidity });
                        break;
                    case "construccion":
                        if (ex.workAddress)
                            rows.push({ label: "Dirección de la obra", value: ex.workAddress });
                        if (ex.duration)
                            rows.push({ label: "Duración estimada", value: ex.duration });
                        if (ex.paymentSchedule)
                            rows.push({ label: "Calendario de pagos", value: ex.paymentSchedule });
                        break;
                    case "eventos":
                        if (ex.eventDate)
                            rows.push({ label: "Fecha del evento", value: ex.eventDate });
                        if (ex.bookingDeposit)
                            rows.push({ label: "Reserva requerida", value: ex.bookingDeposit });
                        if (ex.cancellationPolicy)
                            rows.push({ label: "Política de cancelación", value: ex.cancellationPolicy });
                        break;
                }
                if (ex.notes)
                    rows.push({ label: "Notas adicionales", value: ex.notes });
                if (rows.length === 0)
                    return startY;
                const rowH = 20;
            ...
            if (lines.length === 0) {
                ...
                lines.forEach((line, idx) => {
                    ...
                    doc.fillColor(colors.text).font("Helvetica").fontSize(9)
                        ...
                        .text((0, format_1.formatCurrencyCLP)(line.unitPrice), table.c3, textY, { width: table.pW - 4, align: "right" })
                        .text((0, format_1.formatCurrencyCLP)(line.subtotal), table.c4, textY, { width: table.sW - table.pad, align: "right" });
                    y += rowH;
                });
            }
            y += 18;
            ...
            doc.fillColor(colors.muted).font("Helvetica").fontSize(9)
                ...
                .text((0, format_1.formatCurrencyCLP)(input.total), tx + totW - 60, y, { width: 60, align: "right" });
            doc.fillColor(colors.muted).font("Helvetica").fontSize(9)
            ...
            doc.fillColor(colors.dark).font("Helvetica-Bold").fontSize(12)
                ...
                .text((0, format_1.formatCurrencyCLP)(input.total), tx + totW - 60, y + 42, { width: 60, align: "right" });
            y += 80;
            // ── Extra fields ─────────────────────────────────────────────────────────
            y = drawExtraFields(y);
            // ── Customer notes ────────────────────────────────────────────────────────
            ...
            doc.fillColor(colors.light).font("Helvetica").fontSize(7.5)
                .text(`${brandName}  ·  ${docTitle} ${quoteNumber}  ·  Emitida el ${issueDate}  ·  Documento generado automáticamente`, margin, y + 10, { width: contentWidth, align: "center" });
            doc.end();
